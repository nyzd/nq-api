...
def set_ordering_enum(
    ...
    try:
        ...
        for _param_key, comp_param in components_params.items():
            if (
                isinstance(comp_param, dict)
                and comp_param.get("name") == "ordering"
                and comp_param.get("in") == "query"
            ):
        ...
        for _method, operation in methods.items():
            ...
            for param in parameters:
                ...
                if (
                    isinstance(param, dict)
                    and "$ref" in param
                    and isinstance(param["$ref"], str)
                ):
                    ref = param["$ref"]
                    if ref.startswith("#/components/parameters/"):
                        key = ref.split("/")[-1]
                        comp_param = (
                            result.get("components", {}).get("parameters", {}).get(key)
                        )
                        if (
                            isinstance(comp_param, dict)
                            and comp_param.get("name") == "ordering"
